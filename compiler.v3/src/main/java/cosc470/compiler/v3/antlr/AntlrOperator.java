...
public class AntlrOperator {
    ...
    public AntlrOperator() {
        runTest();
        createEvaluator();
        addEvaluatorToDatabase();
        setDeclaresBlockValues();
        checkTypes();
        runIntermediateCodeGenerator();
        //allOperations();
        //printEvaluator();
    }
    ...
    private void addEvaluatorToDatabase() {
        for (String temp : evaluator) {
            cosc470.compiler.v3.database.Database.addEvaluatorMainItem(temp);
        }
    }

    private void setDeclaresBlockValues() {
        String holder = "";
        for (int i = 0; i < cosc470.compiler.v3.database.Database.getSymbolTableItems().size(); i++) {
            holder = cosc470.compiler.v3.database.Database.getSymbolTableItems().get(i).getValue();
            holder = expressionHandler(holder);
            //System.out.println(holder);
            if (!((holder.equals("NULL")) || (holder.equals("null")))) {
                cosc470.compiler.v3.database.Database.getSymbolTableItems().get(i).setValue(holder);
            }
        }
        //cosc470.compiler.v3.database.Database.printSymbolTableItems();
    }

    private void runIntermediateCodeGenerator() {
        try {
            cosc470.compiler.v3.database.IntermediateCodeGenerator.generateIntermediateCode();
        } catch (Exception ex) {
            System.out.printf("\n\nERROR\nType: %s\nLocation: %s\nThrown Exception: %s\nMessage: %s\nLocalMessage: %s\n", ex.getClass().getName(), ex.getStackTrace()[2], ex.getCause(),
                    ex.getMessage(), ex.getLocalizedMessage());
            // ex.printStackTrace();
